...
class AmphibianViewModel : ViewModel() {
   ...
   private val _status = MutableLiveData<AmphibianApiStatus>()
    ...
    val info : LiveData<List<Amphibian>>
        get() = _amphibians

    // TODO: Create properties to represent MutableLiveData and LiveData for a single amphibian object.
    //  This will be uused to display the details of an amphibian when a list item is clicked
    ...
    fun getAmphibianList(){
        viewModelScope.launch {
            try {
                ...
                // Set the list of amphibians and status on successful response
                _amphibians.value = AmphibianApi.retrofitService.getAmphibians()
                _status.value = AmphibianApiStatus.DONE

            } catch (e:Exception){

                // Handle errors by setting empty list and error status
